...
surveys_weight3060 <- surveys_weight %>% 
  filter(weight > 30)

head(surveys_weight3060,6)
...
biggest_critters <- surveys_weight3060 %>% 
...
surveys_avg_weight <- surveys %>% 
  ...
  mutate(average_wt = ave(weight)) %>% 
  select(species,sex,weight,average_wt)
...
surveys_avg_weight %>% 
  mutate(above_average = weight > average_wt)
...
surveys_extra_challenge <- surveys %>% 
  group_by(species) %>% 
